...
-- Lazy-load localPlayer to avoid circular require during startup on 0.7.x
local localPlayer = nil
local triedLocalPlayer = false
local function getLocalPlayer()
    if localPlayer then
        return localPlayer
    end
    if triedLocalPlayer then
        return nil
    end
    triedLocalPlayer = true
    local ok, lp = pcall(mjrequire, "mainThread/localPlayer")
    if ok then
        localPlayer = lp
        return localPlayer
    end
...
local function getSeason()
    ...
    local playerPosition = lp and lp.getPos and lp:getPos() or nil
    local isSouthHemisphere = playerPosition and (dot(playerPosition, vec3(0.0,1.0,0.0)) < 0.0) or false
...
local function sendNotification(seasonNotifyInfo)
    -- notificationsUI expects the same structure it receives from logicInterface.
    -- Provide a minimal objectSaveData so the UI can render an icon without errors.
